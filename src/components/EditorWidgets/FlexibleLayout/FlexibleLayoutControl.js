...
import SelectControl from 'EditorWidgets/Select/SelectControl';

function FlexibleLayoutItem(props) {
  return <div {...props} className={`list-item ${ props.className || '' }`}>{props.children}</div>;
}
FlexibleLayoutItem.propTypes = {
  className: PropTypes.string,
  children: PropTypes.node,
};
FlexibleLayoutItem.displayName = 'list-item';

function valueToString(value) {
  return value ? value.join(',').replace(/,([^\s]|$)/g, ', $1') : '';
}

const SortableFlexibleLayoutItem = SortableElement(FlexibleLayoutItem);
...
const SortableFlexibleLayout = SortableContainer(({ items, renderItem }) => {
...
export default class FlexibleLayoutControl extends Component {
  static propTypes = {
  ...
  handleLayoutChangeFor(index) {
    return (value) => {
      this.handleChangeFor(index)('layout', value);
    };
  }
  ...
  renderItem = (item, index) => {
    ...
    const selectedLayout = item.get('layout') || field.get('fields').first().get('name');

    const layoutField = field.get('fields').find(item => item.get('name') === selectedLayout);

    const layoutOptions = field.get('fields').map(layout => Map({
      label: layout.get('label'),
      value: layout.get('name'),
    }));

    return (<SortableFlexibleLayoutItem className={classNames.join(' ')} index={index} key={`item-${ index }`}>
      <ListItemTopBar
      ...
      <SelectControl
        ...
        onChange={this.handleLayoutChangeFor(index)}
      />
      <ObjectControl
        value={item}
        field={layoutField}
        ...
        forList
      />
    </SortableFlexibleLayoutItem>);
  };

  renderFlexibleLayoutControl() {
  ...
  render() {
    ...
    if (field.get('field') || field.get('fields')) {
      return this.renderFlexibleLayoutControl();
    }
